...
2. **Use Memory for Context** — Before asking repetitive clarification questions, search memory for related decisions or conventions via `apero prompt-kit memory search --query "<feature/topic>"` and reuse relevant context.
...
7. **Store Reusable Knowledge** — When important conventions or decisions are finalized, store them via `apero prompt-kit memory store --title "<title>" --content "<knowledge>" --tags "<tags>"`.
