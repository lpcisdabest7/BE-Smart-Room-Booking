...
2. **Use Memory for Context** — Search memory for prior decisions that affect priorities/scope: `apero prompt-kit memory search --query "<feature planning updates>"`.
...
5. **Store Reusable Knowledge** — If new planning conventions or risk-handling rules emerge, store them with `apero prompt-kit memory store ...`.
