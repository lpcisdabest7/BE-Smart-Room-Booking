...
1. **Use Memory for Context** — Search memory for related requirements/domain decisions before starting: `apero prompt-kit memory search --query "<feature requirements>"`.
2. Summarize:
...
4. **Store Reusable Knowledge** — If new reusable requirement conventions are agreed, store them with `apero prompt-kit memory store ...`.
