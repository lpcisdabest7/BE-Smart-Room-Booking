...
2. **Use Memory for Context** — Search memory for similar incidents/fixes before deep investigation: `apero prompt-kit memory search --query "<issue symptoms or error>"`.
...
7. **Store Reusable Knowledge** — Save root-cause and fix patterns via `apero prompt-kit memory store ...`.
